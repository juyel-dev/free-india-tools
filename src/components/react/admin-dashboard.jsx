...
const AdminDashboard = () => {
  const { user } = useAuth();
  const { tools, pendingTools, users, approveTool, rejectTool } = useTools();
  ...
  const quickStats = [
    { label: 'Total Tools', value: tools.length, icon: Tool },
    { label: 'Pending Tools', value: pendingTools.length, icon: FileText },
    { label: 'Total Users', value: users.length, icon: Users },
    { label: 'Featured Tools', value: tools.filter((t) => t.featured).length, icon: Star },
  ];

  const quickActions = [
    { label: 'Approve All Pending', icon: Check, action: () => {} },
    { label: 'Add New Banner', icon: ImageIcon, action: () => {} },
    { label: 'Site ON/OFF Toggle', icon: Settings, action: () => {} },
    { label: 'Export Data CSV', icon: Download, action: () => {} },
  ];

  const recentActivity = [
    { type: 'submission', user: 'rajesh', tool: 'Figma Alternative', time: '2 min ago' },
    { type: 'click', tool: 'Photopea', clicks: '1000', time: '1 hour ago' },
    { type: 'signup', user: 'priya@example.com', time: '3 hours ago' },
    { type: 'feature', tool: 'Canva', user: 'admin', time: '1 day ago' },
  ];
